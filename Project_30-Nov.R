...
#########################################################################
# Co-Expression Networks (Binary with Statistical Testing)
#########################################################################
...
cat("Cancer correlations computed ✓\n")
...
cat("Normal correlations computed ✓\n")
...
corr_threshold_n <- 0.75   # Normal correlation threshold (more stringent)
pval_threshold <- 0.01     # Significance threshold (both networks)

cat("\nBuilding binary adjacency matrices...\n")
cat("Cancer - Correlation threshold: |ρ| ≥", corr_threshold_c, "\n")
cat("Normal - Correlation threshold: |ρ| ≥", corr_threshold_n, "\n")
cat("Both networks - Significance threshold: FDR <", pval_threshold, "\n\n")
...
if (density_c > 0.1 || density_n > 0.1) {
  cat("\n⚠️  WARNING: Network is very dense (>10% of possible edges)\n")
  cat("Consider: - Increasing corr_threshold (e.g., 0.75, 0.8)\n")
  cat("          - Decreasing pval_threshold (e.g., 0.001)\n\n")
}

if (density_c < 0.001 || density_n < 0.001) {
  cat("\n⚠️  WARNING: Network is very sparse (<0.1% of possible edges)\n")
  cat("Consider: - Decreasing corr_threshold (e.g., 0.6, 0.65)\n")
