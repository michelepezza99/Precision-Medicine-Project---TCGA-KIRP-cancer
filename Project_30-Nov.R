...
# Strategy: Keep genes with at least 10 counts in 90% of ALL samples
# Total samples = normal + cancer = 32 + 32 = 64
# 90% of 64 = 57.6 → floor = 57
...
# Count how many samples have ≥10 counts for each gene
...
# CoExpression Networks (Binary with Statistical Testing))
...
# Check dimensions
cat("\n=== Co-expression Network Setup ===\n")
cat("Number of DEGs:", nrow(expr_deg_cancer), "\n")
cat("Number of samples (cancer):", ncol(expr_deg_cancer), "\n")
cat("Number of samples (normal):", ncol(expr_deg_normal), "\n")

# Verify alignment
all(rownames(expr_deg_cancer) == rownames(expr_deg_normal))


## Compute correlation WITH statistical testing --------------------------

cat("\nComputing correlations with significance testing...\n")

# CANCER correlation
cor.mat.c <- corr.test(
  t(expr_deg_cancer), 
  use = "pairwise",
  method = "pearson",     # As per project requirements
  adjust = "fdr",         # FDR correction for multiple testing
  ci = FALSE
)

rho.c <- cor.mat.c$r      # Correlation coefficients
diag(rho.c) <- 0          # Remove self-correlations

qval.c <- cor.mat.c$p     # FDR-adjusted p-values
# Mirror p-values to lower triangle
...
# As per project requirements: binary adjacency where aij=0 if |ρ| < threshold
# We use BOTH criteria:
#   1. Statistical significance (FDR-adjusted p-value < 0.01)
#   2. Correlation magnitude (|ρ| ≥ threshold)


# Set thresholds
corr_threshold_c <- 0.7    # Cancer correlation threshold
corr_threshold_n <- 0.8   # Normal correlation threshold (more stringent)
pval_threshold <- 0.05     # Significance threshold (both networks)

# Binary adjacency: 1 if BOTH conditions are met, 0 otherwise
...
# Check positive vs negative (from original correlation)
pos_edges_c <- sum((adj_mat_c == 1) & (rho.c > 0)) / 2
neg_edges_c <- sum((adj_mat_c == 1) & (rho.c < 0)) / 2
cat("  - Positive correlations:", pos_edges_c, "\n")
cat("  - Negative correlations:", neg_edges_c, "\n")
...
pos_edges_n <- sum((adj_mat_n == 1) & (rho.n > 0)) / 2
neg_edges_n <- sum((adj_mat_n == 1) & (rho.n < 0)) / 2
cat("  - Positive correlations:", pos_edges_n, "\n")
cat("  - Negative correlations:", neg_edges_n, "\n")


## IMPORTANT: Check if network is too dense or too sparse ---------------

density_c <- sum(adj_mat_c != 0) / (nrow(adj_mat_c) * (nrow(adj_mat_c) - 1))
density_n <- sum(adj_mat_n != 0) / (nrow(adj_mat_n) * (nrow(adj_mat_n) - 1))

if (density_c > 0.1) {
  cat("\nWARNING (CANCER): Network is very dense (>10% of possible edges)\n")
  cat("Consider:\n")
  cat(" - Increasing corr_threshold (e.g., 0.75, 0.8)\n")
  cat(" - Decreasing pval_threshold (e.g., 0.001)\n\n")
} else if (density_c < 0.001) {
  cat("\nWARNING (CANCER): Network is very sparse (<0.1% of possible edges)\n")
  cat("Consider:\n")
  cat(" - Decreasing corr_threshold (e.g., 0.6, 0.65)\n")
  cat(" - Increasing pval_threshold (e.g., 0.05)\n\n")
} else {
  cat("\nCANCER: Network density is within a normal range:",density_c,"\n")
}

if (density_n > 0.1) {
  cat("\nWARNING (NORMAL): Network is very dense (>10% of possible edges)\n")
  cat("Consider:\n")
  cat(" - Increasing corr_threshold (e.g., 0.75, 0.8)\n")
  cat(" - Decreasing pval_threshold (e.g., 0.001)\n\n")
} else if (density_n < 0.001) {
  cat("\nWARNING (NORMAL): Network is very sparse (<0.1% of possible edges)\n")
  cat("Consider:\n")
  cat(" - Decreasing corr_threshold (e.g., 0.6, 0.65)\n")
  cat(" - Increasing pval_threshold (e.g., 0.05)\n\n")
} else {
  cat("\nNORMAL: Network density is within a normal range:",density_n,"\n")
}
...
cat("\n=== Degree Statistics ===\n")
cat("Cancer network:\n")
cat("  Mean degree:", round(mean(degree_c), 2), "\n")
cat("  Median degree:", median(degree_c), "\n")
cat("  Max degree:", max(degree_c), "\n")
cat("  Isolated nodes (degree=0):", sum(degree_c == 0), "\n")

cat("\nNormal network:\n")
cat("  Mean degree:", round(mean(degree_n), 2), "\n")
cat("  Median degree:", median(degree_n), "\n")
cat("  Max degree:", max(degree_n), "\n")
cat("  Isolated nodes (degree=0):", sum(degree_n == 0), "\n")
...
# Cancer log-log plot (scale-free check)
...
if (length(deg_freq_c) > 1) {
  plot(log10(as.numeric(names(deg_freq_c))), log10(as.numeric(deg_freq_c)), 
       ...
       pch = 16, col = "lightblue", cex = 0.5)
  # Add linear fit to check power-law
...
cat("\n=== Hub Analysis ===\n")
cat("Cancer hub threshold (95th percentile):", round(x, 2), "\n")
cat("Normal hub threshold (95th percentile):", round(y, 2), "\n")
...
cat("\nHub comparison:\n")
cat("  Cancer hubs:", length(hubs_c), "\n")
cat("  Normal hubs:", length(hubs_n), "\n")
cat("  Common hubs:", length(common_hubs), "\n")
cat("  Cancer-selective hubs:", length(selective_hubs_c), "\n")
cat("  Normal-selective hubs:", length(selective_hubs_n), "\n")

cat("\nTop 10 cancer hubs:\n")
print(head(hubs_c, 10))

cat("\nTop 10 normal hubs:\n")
print(head(hubs_n, 10))

#########################################################################
# Output:
#   adj_mat_c = BINARY adjacency matrix for cancer co-expression network
#   adj_mat_n = BINARY adjacency matrix for normal co-expression network
#   rho.c, rho.n = correlation matrices (for reference)
...
z_threshold <- 5 # SCELTO PIU' RESTRITTIVO PER FAR VENIRE RETE MENO DENSA -> POWER-LAW +-
...
if (length(deg_freq_diff) > 1) {
  ...
  #fit_z <- lm(log10(as.numeric(deg_freq_diff)) ~ log10(as.numeric(names(deg_freq_diff))))
  #abline(fit_z, col = "lightcoral", lwd = 2, lty = 2)
  #legend("topright", legend = paste("R² =", round(summary(fit_z)$r.squared, 3)),
  #      bty = "n")
}
