...
```{r}
threshold_plot <- 0.7 
patient_sim_matrix[patient_sim_matrix < threshold_plot] <- 0
```
...
```{r}
net_final <- network(patient_sim_matrix, 
                     matrix.type = "adjacency", 
                     ignore.eval = FALSE, 
                     names.eval = "weights", 
                     directed = FALSE)
...
```{r}
threshold_plot <- 0.7 
psn_normal_matrix[psn_normal_matrix < threshold_plot] <- 0
```
...
```{r}
net_normal <- network(psn_normal_matrix,
                      matrix.type = "adjacency",
                      ignore.eval = FALSE,
                      names.eval = "weights",
                      directed = FALSE)
...
```{r}
net_snf <- network(W_fused, matrix.type = "adjacency", ignore.eval = FALSE, names.eval = "weights", directed = FALSE)
```
...
![](images/clipboard-2023213764.png)
